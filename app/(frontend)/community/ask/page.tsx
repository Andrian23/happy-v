...
import { countAskTopics, getTopicsByAsk } from "@/actions/topic"
...
const AskCommunityPage = () => {
  const [totalItemCount, setTotalItemCount] = useState(0)
  const [topicAskCount, setTopicAskCount] = useState(0)
  const [activeItem, setActiveItem] = useState(0)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [topics, setTopics] = useState<Topic[]>([])
  const tabs = useMemo(() => [`Latest (${topicAskCount})`, "Top", "My posts"], [topicAskCount])
  ...
  useEffect(() => {
    const fetchTopics = async () => {
      try {
        const data = await getTopicsByAsk()
        setTopics(data)
      } catch (error) {
    ...
    const fetchTopicAskCount = async () => {
      try {
        const count = await countAskTopics()
        setTopicAskCount(count)
      } catch (error) {
