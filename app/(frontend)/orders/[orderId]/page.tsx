...
const OrderPage: React.FC<OrderPageProps> = () => {
  ...
  const [loading, setLoading] = useState(true) // Додано стан loading
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        if (orderIndex !== -1 && parts[orderIndex + 1]) {
          const orderId = parts[orderIndex + 1]
          const orderData = await getOrderByOrderId(parseInt(orderId, 10)) // Convert orderId to integer
          setOrder(orderData)
        } else {
              ...
              <div className="mt-2 flex justify-between">
                <div className="">
                  ...
                  <div className="mt-2 flex text-sm">
                    ...
                    <div className="mx-1 text-primary-900">{formatDate(order.createdAt)}</div>
                ...
                <div className="flex h-auto w-full justify-between max-lg:p-0 lg:p-4">
                  ...
                  <div className="w-[40%] bg-grey-200 p-4 text-sm max-lg:rounded-s-none max-md:w-full lg:rounded-2xl">
                    ...
                    <div className="mt-[16px] flex justify-between text-sm text-grey-800">
                      <div className="">{order.shippingMethod.type}</div>
                      <div className="">{order.shippingMethod.price}</div>
                    </div>
                    <div className="mt-[16px] flex justify-between text-sm text-grey-800">
