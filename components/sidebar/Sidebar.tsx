...
import { getAmbassadorCounts } from "@/actions/super-admin/counts"
...
const Sidebar: React.FC<SidebarProps> = ({ isAdmin = false }) => {
  const [counts, setCounts] = useState<{
    pendingReviewAmbassadors: number
    activeAmbassadors: number
    declinedRequestsAmbassadors: number
    pendingReviewPartners: number
    activePartners: number
    declinedRequestsPartners: number
  }>({
    pendingReviewAmbassadors: 0,
    activeAmbassadors: 0,
    declinedRequestsAmbassadors: 0,
    pendingReviewPartners: 0,
    activePartners: 0,
    declinedRequestsPartners: 0,
  })
  ...
  useEffect(() => {
    ...
    return bindToEvent("admin-dashboard", "counts-updated", fetchCounts)
  }, [])
                      ...
                      <AccordionItem value="ambassadors" className="mx-4 rounded-lg">
                        <AccordionTrigger className="md:[&>svg]:stroke-light-grey md:text-light-grey cursor-pointer px-2.5 py-2 text-sm text-black [&>svg]:h-5 [&>svg]:w-5 [&>svg]:stroke-black">
                          <div className="relative flex items-center gap-2 md:gap-2.5">
                            {item.name === "Ambassadors hub" && counts && counts.pendingReviewAmbassadors > 0 && (
                              <div className="bg-notified absolute top-0 -right-3 aspect-square w-1.5 rounded-full" />
                            )}
                            {item.name === "Partners hub" && counts && counts.pendingReviewPartners > 0 && (
                              <div className="bg-notified absolute top-0 -right-3 aspect-square w-1.5 rounded-full" />
                            )}
                            {item.icon}
                            {item.name}
                        ...
                        <AccordionContent className="w-full pb-0 text-sm">
                          {item.subLinks.map(({ name, icon, link, countType }) => (
                            <SidebarLink
                              key={link}
                              link={link}
                              icon={icon}
                              ...
                              isBadged={!!countType}
                              isNotified={
                                countType === "pendingReviewAmbassadors" || countType === "pendingReviewPartners"
                              }
                              count={countType ? counts[countType as keyof typeof counts] : 0}
                              countType={countType}
                            />
                          ))}
                        </AccordionContent>
